...
{ 
   ...
   * (named in relation to how data move for child's perspective).
  ...
  if ((pid=fork()) == 0) { 
     ...
     */ 
    close(in[1]);  // 1-directional pipe: child READS from parent from here 
    close(out[0]); // 1-directional pipe: child WRITES to parent from here
     ...
     * ie. If any of the exec() functions returns, an error will have occurred. 
     */
  ...
  /* Message indicating what the spawned child process
   * will be used for.
   */
   ...
   */ 
  close(in[0]);  // 1-directional pipe: parent WRITES to child from here 
  close(out[1]); // 1-directional pipe: parent READS from child from here
